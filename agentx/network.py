...
class Network:
    def __init__(self, server_address="/var/agentx/master", debug=False, timeout=1.0):
        ...
        self._timeout = timeout  # Seconds (increased from 0.1 to 1.0 for better reliability)

    def connect(self):
        if self._connected:
            return

        # Try TCP connection first if address is host:port format
        if not self._server_address.startswith("/"):
            try:
                ...
                logger.info("Connected to %s (TCP)" % self._server_address)
                return
            except socket.error as e:
                logger.debug("TCP connection to %s failed: %s, trying Unix socket" % (self._server_address, e))
                # Fall back to Unix socket
                try:
                    self.socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
                    self.socket.connect("/var/agentx/master")
                    self.socket.settimeout(self._timeout)
                    self._connected = True
                    logger.info("Connected to /var/agentx/master (Unix socket)")
                    return
                except socket.error as e2:
                    self._connected = False
                    logger.error("Failed to connect to %s (TCP) or /var/agentx/master (Unix): %s" % (self._server_address, e2))
                    return
        ...
        try:
            ...
            self.socket.settimeout(self._timeout)
            self._connected = True
            logger.info("Connected to %s (Unix socket)" % self._server_address)
        except socket.error:
            self._connected = False
